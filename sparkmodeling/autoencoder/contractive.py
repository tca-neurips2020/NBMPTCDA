...
class CAE:
    ...
    def log_losses(self, X, val=False, verbose=False, e=0):
        ...
        loss = self.eval_var(self.loss, X)
        logging.debug("[debug]: comp1: {:.5f} \t comp2: {:.5f}".format(
            loss, recons_loss + self.lamda * jacobian_loss))
    ...
    def fit_batches(
            self, batches, val_batches=None, log_time=False, verbose=False):
        t0 = time.time()
        ...
        if self.early_stopping:
            assert val_batches is not None
        ...
        self.log_losses_batch(batches, verbose=verbose)

        if self.early_stopping:
            self.log_losses_batch(val_batches, val=True, verbose=verbose)
        ...
        for e in range(self.n_epochs):
            ...
            for key in batches:
                ...
                self.train_step.run(feed_dict=self.get_fd(X_batch),
                                    session=self.session)
            loss_value = self.log_losses_batch(batches, verbose=verbose, e=e)
            if self.early_stopping:
                val_loss_value = self.log_losses_batch(
                    val_batches, val=True, verbose=verbose, e=e)
    ...
    def fit(self, X, log_time=False, verbose=False):
        t0 = time.time()
        ...
        sess = self.session
        sess.run(tf.compat.v1.global_variables_initializer())
        self.log_losses(X, verbose=verbose)

        if self.early_stopping:
            self.log_losses(X_val, val=True, verbose=verbose)
        ...
        for e in range(self.n_epochs):
            ...
            for i in range(n_batches):
                ...
                self.train_step.run(feed_dict=self.get_fd(x_batch),
                                    session=self.session)
            loss_value = self.log_losses(X, verbose=verbose, e=e)
            if self.early_stopping:
                val_loss_value = self.log_losses(
                    X_val, val=True, verbose=verbose, e=e)
                if val_loss_value < min_err:
                    min_err = val_loss_value
                    best_epoch = e
                    self.best_epoch_ = e
    ...
    def build(input_dim=561,
        ...
        ae = CAE(input_dim, hidden_layer_sizes, activations, lamda=lamda,
