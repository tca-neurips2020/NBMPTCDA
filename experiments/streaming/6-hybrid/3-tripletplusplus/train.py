...
def train_model_and_evaluate(
    ...
    autoencoder_params['input_dim'] = lods.trainval.Y.shape[1]  # 561 metrics
...
def main():
    ...
    logging.info("Number of triplets: {}".format(len(triplet_idxs)))
    ...
    for i in range(N_WORKERS):
        ...
        p.start()
        os.system("taskset -p -c %d %d" % (((i+10) % os.cpu_count()), p.pid))
        processes.append(p)
