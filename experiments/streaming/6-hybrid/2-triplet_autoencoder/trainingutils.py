...
def extract_and_set_encoding(autoencoder, trace):
    ...
    alias, missing_centroid = extract_encoding(
        autoencoder, trace, use_full_trace=False)
...
def extract_encoding_and_map_to_nearest(
    ...
    """
    _, _, debug = trace
    autoencoder_centroids = autoencoder.centroids.copy()
    logging.info("Trace's Y shape: {}".format(debug.shape))

    # Step1: Extract encoding from the input trace
    alias, missing_centroid = extract_encoding(
        autoencoder, trace, use_full_trace=False)
    if alias in autoencoder_centroids:
        del autoencoder_centroids[alias]
    ...
    if within_template:
        ...
        aliases_to_check = sorted(list(autoencoder_centroids.keys()))
    ...
    for a in aliases_to_check:
        centroids_to_compare.append(autoencoder_centroids[a])
    ...
    autoencoder.altered_centroids[alias] = autoencoder_centroids[proxy_a].copy()
