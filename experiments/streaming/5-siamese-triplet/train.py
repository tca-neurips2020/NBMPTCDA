...
from sparkmodeling.autoencoder.tripletencoder import TEncoder
...
class OneWorkerProcess(Process):
    def __init__(self, lods, triplets, q, out_data):
    ...
    def run(self):
        ...
        while True:
            ...
            train_model_and_evaluate(
                lods, self.triplets, self.out_data, seed=seed)


def evaluate(test_job, encoder,
    ...
    extract_and_set_encoding(encoder, trace)
    proxy = extract_encoding_and_map_to_nearest(
        encoder, trace, lods.alias_to_id, test_aliases)
...
def fetch_triplets(triplets_idxs, lods):
    ...
    return get_anchor(
        lods, anchor_idxs).Y, get_pos(
        lods, pos_idxs).Y, get_neg(
        lods, neg_idxs).Y
...
def get_triplets(lods, with_shuffle=True, random_state=42):
    triplets_idxs = get_triplet_idxs(
        lods, with_shuffle=with_shuffle, random_state=random_state)
    return fetch_triplets(triplets_idxs, lods)
...
def compute_centroids(encoder, lods, scheme='shared'):
    ...
    for a in slices:
        idxs = slices[a]
        encodings = encoder.transform(ds.Y[idxs, :])
...
def train_model_and_evaluate(lods, triplets, out_data, seed=10):
    ...
    # 2. train the encoder on the triplets
    encoder_params = HYPER_PARAMS['encoder_params']
    encoder_params['input_dim'] = Ya.shape[1]  # 561 metrics

    encoder = TEncoder(v1_compat_mode=True, **encoder_params)
    encoder.compile()
    encoder.centroids = None
    encoder.altered_centroids = None
    encoder.fit(Ya, Yp, Yn, log_time=True)

    # 3. extract encodings for training workloads
    compute_centroids(encoder, lods, scheme=ENCODING_SCHEME)
    ...
    X, y = translate_X_y(X_, y_, encoder.centroids)
    ...
    logging.info(
        "Fitting regressor on data of shapes: {}, {}".format(
            X.shape, y.shape))
    reg = NNregressor(**nn_params, v1_compat_mode=True, random_state=seed)
    ...
    for test_job in observed_traces_slices:
        evaluate(test_job, encoder, reg, lods, observed_traces,
                 observed_traces_slices, out_data, test_aliases)

    # Append trained encoder information (with centroids) to output_data
    out_data['encoders'].append(encoder.get_persist_info())
