...
from sparkmodeling.autoencoder.snnae import SNNAE
...
def evaluate(test_job, snnae,
    ...
    extract_and_set_encoding(snnae, trace)
    proxy = extract_encoding_and_map_to_nearest(
        snnae, trace, lods.alias_to_id, test_aliases)
...
def main():
    ...
    for i in range(N_WORKERS):
        ...
        os.system("taskset -p -c %d %d" %
                  (((0*N_WORKERS+i) % os.cpu_count()), p.pid))
        processes.append(p)
